...
---
### My thoughts
- Hardcoded date (2022-01-01), could this be a variable?
- This seems to be alot of rows to be removed, `Rows Removed by Join Filter: 361371`
- Check if this can optimized so that we do not have to build and drop 300K rows, everytime.

### Ai Enhancement
The 361,371 removed rows are the result of generate_series producing every date
since 2022-01-01 being matched against every row in aggregate_price_graph (739 rows)
as a join filter rather than a proper join condition — forcing the database to evaluate
every combination before discarding mismatches.

The hardcoded 2022-01-01 makes this actively worse over time — the date series grows
by one day every day, increasing the throwaway row count indefinitely.

Optimisations:
- Replace the hardcoded date with a dynamic start based on actual data range
- Restructure generate_series as a CTE so dates are generated once and joined cleanly
- Consider pre-aggregating price data on write to avoid this computation at read time